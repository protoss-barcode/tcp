...
(deftest test-*
  ...
  (testing "Exceptional behavior"
    ...
    ;; BUG: `ArithmeticException` is not thrown
    (is (= ##Inf (core/* Double/MAX_VALUE 2)))
...
(deftest test-*'
  ...
  (testing "`(* x y & more)` returns the product of all arguments"
    ...
    (is (= 1 (apply core/*' (take 100 (repeat 1))))
        "Arity 3+"))
  (testing "If the value of the result exceeds `java.lang.Long/MAX_VALUE`, it is promoted to a `java.math.BigInteger`"
    (is (= 18446744073709551614N (core/*' Long/MAX_VALUE 2)))
    (is (instance? clojure.lang.BigInt (core/*' Long/MAX_VALUE 2))))
  (testing "Exceptional behavior"
    ;; BUG: Value is not promoted to `java.math.BigDecimal
    (is (= ##Inf (core/* Double/MAX_VALUE 2)))
    (is (thrown? ClassCastException (core/*' "foo")))
    (is (thrown? ClassCastException (core/*' 1 "a")))
    (is (thrown? ClassCastException (core/*' :bar)))))
